...
my @files = sort {$a<=>$b} glob("fragments/*.fas");

my %ffiles;

foreach my $file (@files){
  if ($file =~ /(fragments\/spike14_\d+_\d+\.fas)/){
    my $f = $1;
    $file =~ /fragment_(\d+)_\d+\.fas/;
    $ffiles{$f} = $1;
  }
}
...
for (my $i = 0; $i <= $#ffiles; $i++){
	...
	my $X = ();
	my $Y = ();
	if ($ffiles[$i] =~ /spike14_([\d]+)_([\d]+).fas/){
		$X = $1;
		$Y = $2;
	} else {
	...
	for (my $j = $i+1; $j <= $#ffiles; $j++){
		...
		my $Z = ();
		my $W = ();
		if ($ffiles[$j] =~ /spike14_([\d]+)_([\d]+).fas/){
			$Z = $1;
			$W = $2;
			my $outfile = 'spike14_'.$X.'_to_'.$Y.'_AND_'.$Z.'_to_'.$W;
			print ("\t->$outfile\n");
...
sub concatena {
	...
	my $X = ();
	my $Z = ();
	if ($_[2] =~ /spike14_(\d+)_to_\d+_AND_(\d+)_to_\d+/){
		$X = $1;
		$Z = $2;
	} else {
